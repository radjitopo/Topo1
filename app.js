...
const experienceIntros = Object.freeze({
  topo: {
    label: 'VOTAR',
...
function experienceIntroHTML(kind) {
  const intro = experienceIntros[kind];
  return intro
...
function pageKind() {
  ...
  if (/^\/(?:rankings|descobrir)(?:\/[^/]+)?\/?$/.test(location.pathname)) return 'discover';
  if (
...
function discoverHomeCalloutHTML() {
  return `<section class="discoverHomeCallout" aria-labelledby="discover-home-title"><header><div><span class="discoverEyebrow">RANKINGS EDITORIAIS</span><h2 id="discover-home-title">Rankings</h2><p>Informação clara, números reais, data e fonte — sem votação.</p></div><a href="/rankings">VER OS 139 RANKINGS →</a></header><div class="discoverHomePreview"><span class="discoverStatus">139 PUBLICADOS</span><div><h3>De Brasil, cinema e música a tecnologia, viagens, gastronomia e velocidade.</h3><p>Top 10 completo, com o valor de cada posição.</p></div><a href="/rankings" aria-label="Abrir Rankings">↗</a></div></section>`;
}
...
function renderDiscoverPage() {
  if (feed.dataset.serverRendered === 'true') {
    ...
    if (activeCategory)
      requestAnimationFrame(() => {
        const rail = activeCategory.parentElement;
        if (rail)
          rail.scrollLeft =
            activeCategory.offsetLeft - (rail.clientWidth - activeCategory.offsetWidth) / 2;
      });
    return;
  }
  document.title = 'Rankings editoriais — TOPO';
  feed.innerHTML = `<section class="portalEmpty"><h1>Rankings</h1><p>Abra novamente a seção para carregar os rankings editoriais.</p><a class="categoryVoteCta" href="/rankings">Ver os 139 rankings →</a></section>`;
}
...
function searchRelevance(r) {
  ...
  if (titlePhrase === phrase) return 7;
  if (titlePhrase.startsWith(phrase)) return 6;
  if (titlePhrase.includes(phrase)) return 5;
