...
function rankingEvaluationProgressHTML(r) {
  const available = Math.min(Number(viewer.rankingLimit || 20), r.opts.length),
    done = Math.min(myVoteCount(r), available),
    remaining = Math.max(0, available - done),
    percent = available ? Math.round((done / available) * 100) : 0;
  return `<section class="rankingEvaluationProgress" aria-label="${done} de ${available} opções avaliadas"><div class="rankingEvaluationCopy"><span>Suas avaliações</span><strong>${done} de ${available}</strong></div><div class="rankingEvaluationTrack"><span style="width:${percent}%"></span></div><small>${remaining ? `Faltam ${remaining} para você organizar todo o ranking.` : 'Você avaliou o ranking inteiro.'}</small></section>`;
}
