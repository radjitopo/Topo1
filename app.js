Warning: truncated output (original token count: 72318)
Total output lines: 5727
...
async function rankingOptionPromotionCanvas(r, option) {
  ...
  return canvas…22318 tokens truncated…       (result.ranking?.opts || []).map((option) => [Number(option.id), option.label]),
      );
