...
function popHomeHeroHTML(featured) {
  ...
  const firstPath = cards[0] ? rankingPath(cards[0].id) : '#para-voce';
  return `<section class="popHomeHero"><div class="popHomeHeroCopy"><span class="popEyebrow">SUA VEZ</span><h1>Vote e <em>mude o topo.</em></h1><p>Escolha um ranking, entre na disputa e mude a ordem.</p><div class="popHomeActions"><a class="popPrimaryAction" href="${firstPath}">VOTAR AGORA</a><a class="popSecondaryAction" href="/local">TOPO LOCAL</a></div><div class="popHomePulse"><strong>${fmt(community.rankings)}</strong><span>rankings ativos</span><i></i><strong>${fmt(community.votes)}</strong><span>votos da comunidade</span></div></div><div class="popHomeStack" aria-label="Rankings em destaque"><span class="popHomeStackLabel">AGORA NO TOPO</span>${cards.map(popHomeCardHTML).join('')}<span class="popSpark one" aria-hidden="true">✦</span><span class="popSpark two" aria-hidden="true">↗</span></div></section>`;
}
