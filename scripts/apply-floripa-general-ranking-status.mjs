...
const MIGRATION_KEY = '20260908_deactivate_floripa_general_rankings';
const TARGET_RANKING_IDS = ['bandas-ilha-da-magia', 'bandas-rock-ilha-da-magia'];
...
const migration = await readFile(
  new URL('../migrations/20260908_deactivate_floripa_general_rankings.sql', import.meta.url),
  'utf8',
);
const statements = [
...
if (
  ...
  throw new Error(`Florianópolis general ranking validation failed: ${JSON.stringify(validation)}`);
}

console.log(
  `${TARGET_RANKING_IDS.length} rankings locais removidos do TOPO; ${validation.option_count} opções e toda a participação preservadas.`,
);
