...
test('the local seed is a complete 21 by 15 matrix', () => {
  assert.equal(catalog.length, 315);
  assert.equal(new Set(catalog.map((ranking) => ranking.id)).size, 315);
  ...
  for (const city of local.cityOrder) {
    ...
    assert.equal(cityRankings.length, 15, city);
    assert.deepEqual(
...
test('all 6,281 starting options are usable and unique inside each ranking', () => {
  assert.equal(
