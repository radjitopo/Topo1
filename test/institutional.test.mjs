...
assert.deepEqual(Object.keys(pages), expectedSlugs, 'the institutional catalog must remain complete and ordered');
...
for (const slug of expectedSlugs) {
  ...
  assert.ok(res.body.includes('/style.css?v=20260823-12'));
  ...
  for (const link of expectedLinks) assert.ok(res.body.includes(link), `${slug} footer must include ${link}`);
}
...
for (const link of expectedLinks) assert.ok(index.includes(link), `home footer must include ${link}`);
for (const slug of expectedSlugs) {
