...
import {
  optionSimilarity,
  possibleOptionDuplicate
} from '../option-similarity.js';
...
assert.equal(optionSimilarity('Café', 'cafe'), 1, 'accents and case must not hide exact duplicates');
assert.ok(optionSimilarity('Chave Philips', 'Chave Phillips') > 0.9, 'one-letter spelling variations must score highly');
assert.deepEqual(
