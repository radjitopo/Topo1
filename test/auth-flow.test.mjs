...
test('Clerk keeps the passwordless TOPO UI and OAuth callback inside SomosTopo', async () => {
  const [app, page] = await Promise.all([
    readFile(new URL('app.js', root), 'utf8'),
    readFile(new URL('page.js', root), 'utf8'),
  ]);

  assert.doesNotMatch(app, /clerk\.mountSignIn\(mount,/);
  assert.doesNotMatch(app, /@clerk\/ui|@clerk\/localizations|__internal_ClerkUICtor/);
  assert.doesNotMatch(app, /initClerk\(true\)/);
  assert.match(app, /const clerk = await initClerk\(\),/);
