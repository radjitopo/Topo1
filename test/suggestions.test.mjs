...
assert.match(api, /const approvedRankingTitle = kind === 'ranking' && decision === 'approve'/, 'ranking approval must receive the final title');
assert.match(api, /const approvedRankingCategory = kind === 'ranking' && decision === 'approve'/, 'ranking approval must receive the editorial category');
assert.match(api, /SET title = \$3,[\s\S]*normalized_title = \$4,[\s\S]*category = \$5,[\s\S]*status = 'approved'/, 'manual approval must save title and category together');
assert.doesNotMatch(api, /decision === 'generate'/, 'manual mode must not expose an unavailable AI generation action');
...
assert.match(app, /function moderationRankingReviewHTML\(item\)/, 'ranking ideas must expose a focused editorial approval form');
assert.match(app, /data-ranking-title/, 'moderators must be able to adjust the final ranking title');
assert.match(app, /data-ranking-category/, 'moderators must choose the ranking category before approval');
...
assert.match(app, /Você não precisa montar nem revisar o Top 20/, 'the queue must explain that the team handles the remaining work');
const moderationRender = app.slice(app.indexOf('async function renderModeration'), app.indexOf('async function boot'));
assert.doesNotMatch(moderationRender, /bindModerationDrafts|bindModerationPublishForms/, 'opening moderation must not trigger unavailable automatic generation');
...
assert.match(style, /\.moderationRankingReview\{/, 'manual title and category review must be styled');
assert.match(style, /\.moderationCreationReady\{/, 'the manual creation queue state must be styled');
