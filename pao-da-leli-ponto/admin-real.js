...
async function render(){
  try{
    ...
    $('#pendentes').textContent=corrections.filter(c=>c.status==='pending').length;
    ...
    $('#employeeList').innerHTML=empUsers.map(u=>'<div class="item"><div><strong>'+esc(u.name)+'</strong><div class="sub">'+esc(u.email)+' · '+esc(u.position)+' · '+esc(u.unit)+'</div><div style="margin-top:6px">'+(u.pending_activation?'<span class="badge pending">aguardando ativação</span><div class="sub" style="margin-top:7px">Código: <strong style="font-size:15px;color:var(--wine)">'+esc(u.activation_code||'—')+'</strong> <button class="btn small secondary" style="margin-left:6px" onclick="copyActivation(\''+esc(u.activation_code||'')+'\')">Copiar</button></div>':u.active?'<span class="badge approved">ativo</span>':'<span class="badge rejected">desativado</span>')+'</div></div><div class="actions">'+(u.pending_activation?'<button class="btn small secondary" onclick="regenActivation(\''+u.id+'\',\''+esc(u.email)+'\')">Trocar código</button>':'')+'<button class="btn small secondary" onclick="toggleUser(\''+u.id+'\')">'+(u.active?'Desativar':'Reativar')+'</button></div></div>').join('')||'<p class="muted">Nenhum funcionário.</p>';

    const labels={in:'Entrada',breakOut:'Saída para intervalo',breakIn:'Volta do intervalo',out:'Saída / fim da jornada'};
    $('#correctionList').innerHTML=corrections.map(c=>'<div class="item"><div><strong>'+esc(c.name)+' — '+labels[c.kind]+'</strong><div class="sub">'+c.work_date.split('-').reverse().join('/')+' · original <b>'+time(c.original_at)+'</b> → solicitado <b>'+time(c.requested_at)+'</b></div><div class="sub">Motivo: '+esc(c.reason)+'</div><div style="margin-top:6px"><span class="badge '+c.status+'">'+(c.status==='pending'?'pendente':c.status==='approved'?'aprovada':'recusada')+'</span>'+(c.decided_by_name?'<span class="sub"> · por '+esc(c.decided_by_name)+'</span>':'')+'</div></div>'+(c.status==='pending'?'<div class="actions"><button class="btn small" onclick="decide(\''+c.id+'\',\'approved\')">Aprovar</button><button class="btn small red" onclick="decide(\''+c.id+'\',\'rejected\')">Recusar</button></div>':'')+'</div>').join('')||'<p class="muted">Nenhuma solicitação de correção.</p>';
...
window.regenActivation=async(id,email)=>{try{const r=await api('admin-reset-activation','POST',{id});alert('Novo código para '+email+':\n\n'+r.activationCode+'\n\nEnvie este código à pessoa.')}catch(e){alert(e.message)}}
window.decide=async(id,status)=>{const note=prompt(status==='approved'?'Observação opcional da aprovação:':'Motivo opcional da recusa:','')??'';try{await api('admin-decide-correction','POST',{id,status,note});await render()}catch(e){alert(e.message)}}
...
$('#adminLoginForm').addEventListener('submit',async e=>{e.preventDefault();try{const r=await api('login','POST',{email:$('#adminEmail').value.trim(),password:e.target.querySelector('input[type=password]').value});if(r.user.role!=='admin'){await api('logout','POST',{});throw new Error('Este usuário não é administrador.')}currentUser=r.user;showOnly('dashboard');$('#loggedAs').textContent='Entrou como '+currentUser.name;await render()}catch(err){alert(err.message)}});
$('#employeeForm').addEventListener('submit',async e=>{e.preventDefault();try{const r=await api('admin-create-user','POST',{name:$('#empName').value.trim(),email:$('#empEmail').value.trim(),position:$('#empRole').value.trim()||'Funcionária',unit:$('#empUnit').value.trim()||'Pão da Leli',role:'employee'});$('#activationResult').innerHTML='<div class="notice" style="margin-top:12px"><b>'+esc(r.user.name)+'</b><br>Código de ativação: <strong style="font-size:18px">'+esc(r.activationCode)+'</strong><br><span class="sub">Este código ficará visível aqui até a conta ser ativada.</span></div>';e.target.reset();$('#empRole').value='Funcionária';$('#empUnit').value='Pão da Leli';await render()}catch(err){alert(err.message)}});
