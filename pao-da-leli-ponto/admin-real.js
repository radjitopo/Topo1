...
function formatPhone(v){const d=String(v||'').replace(/\D/g,'');if(d.length===11)return'('+d.slice(0,2)+') '+d.slice(2,7)+'-'+d.slice(7);if(d.length===10)return'('+d.slice(0,2)+') '+d.slice(2,6)+'-'+d.slice(6);return String(v||'')}
function contactLabel(person){return person?.phone?formatPhone(person.phone):(person?.email||'Sem contato')}
...
window.regenActivation=async id=>{const person=overview?.users?.find(u=>u.id===id),contact=contactLabel(person)||'esta pessoa',alreadyPending=Boolean(person?.pending_activation);if(!confirm(alreadyPending?'Trocar o código de '+contact+'? O código anterior deixará de funcionar.':'Gerar um código para '+contact+' criar outra senha? A senha atual será invalidada e as sessões abertas serão encerradas.'))return;try{const r=await api('admin-reset-activation','POST',{id});await render();alert('Novo código de senha: '+r.activationCode)}catch(e){alert(e.message)}}
...
$('#adminLoginForm').addEventListener('submit',async e=>{e.preventDefault();try{const r=await api('login','POST',{identifier:$('#adminIdentifier').value.trim(),password:$('#adminPassword').value});if(r.user.role!=='admin'){await api('logout','POST',{});throw new Error('Este usuário não é administrador.')}currentUser=r.user;showOnly('dashboard');$('#loggedAs').textContent='Entrou como '+currentUser.name;await render();await activateRequestedAdminTab()}catch(err){alert(err.message)}});
$('#adminForgotPassword').addEventListener('click',async()=>{const identifier=prompt('Informe o telefone do administrador (ou o e-mail antigo):',$('#adminIdentifier').value.trim());if(identifier===null)return;try{await api('request-password-reset','POST',{identifier:identifier.trim()});alert('Pedido enviado. Outro administrador deve gerar o novo código. Depois use “Primeiro acesso / ativar conta” na tela do ponto para criar outra senha.')}catch(err){alert(err.message)}});
